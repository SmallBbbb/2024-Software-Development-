...
(
    ...
    CAPTAIN_NUMBER    varchar(50)                                                          not null,
    TASK_RELEASE_TIME timestamp                                  default CURRENT_TIMESTAMP not null,
    TASK_LOCATION     varchar(150)                                                         not null,
    TASK_DESCRIPTION  varchar(500)                                                         not null,
    TASK_STATE        enum ('RELEASED', 'EXECUTING', 'COMPLETE') default 'RELEASED'        not null,
...
(
    ...
    PET_AGE              int          null,
    PET_ADOPTER_ID       varchar(50)  not null,
    PET_HEALTH_CONDITION varchar(500) not null,
...
(
    ...
    RELY_TIME           timestamp                                                          null,
    ADOPTION_SKETCH     varchar(500)                                                       not null,
    CHECK_STATE         enum ('UNCHECKED', 'PASSED', 'REJECTED') default 'UNCHECKED'       not null,
...
(
    ...
    constraint TASK_MUMBER_task_TASK_NUMBER_fk
        foreign key (TASK_NUMBER) references task (TASK_NUMBER),
    constraint task_mumber_stuff_STUFF_ID_NUMBER_fk
        foreign key (MEMBER_NUMBER) references stuff (STUFF_ID_NUMBER),
    constraint task_mumber_user_USER_ID_NUMBER_fk
        foreign key (MEMBER_NUMBER) references user (USER_ID_NUMBER)
);
